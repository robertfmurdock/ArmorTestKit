...
public func assertSame <T : AnyObject> (expected: T, _ actual: T, file: String = __FILE__, line: UInt = __LINE__ ) {
    XCTAssert(expected === actual, "Expected: \(expected) was not: \(actual)", file: file, line: line)
}

public func assertSame <T : AnyObject, K> (expected: T, _ actual: K!, file: String = __FILE__, line: UInt = __LINE__ ) {
    if let checkedActual = actual as? T {
        XCTAssert(expected === checkedActual, "Expected: \(expected) was not: \(checkedActual)", file: file, line: line)
    } else {
...
public func assertEquals (expected: Double, _ actual : Double, delta: Double, file: String = __FILE__, line: UInt = __LINE__ ) {
    XCTAssert(abs(expected - actual) <= delta, "Actual: \(actual) was not within \(delta) of \(expected)", file: file, line: line)
}

public func assertEqualsImplementation <T : Equatable> (original : T, _ equal : T, _ notEqualObjects : [T], file: String = __FILE__, line: UInt = __LINE__) {
